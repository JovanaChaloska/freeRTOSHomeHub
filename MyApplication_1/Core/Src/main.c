...
/* USER CODE BEGIN PV */
extern xQueueHandle TempQueueHandle;
extern xQueueHandle LightQueueHandle;
extern xQueueHandle PollutionQueueHandle;
extern xQueueHandle IsHomeQueueHandle;
extern xTaskHandle xTempSensor;
...
extern void TemperatureTask(void *argument);
extern void LightTask(void *argument);
extern void PollutionTask(void *argument);
extern void IsHomeTask(void *argument);
	...
	/* Infinite loop */
		for(;;)
		{
			for (int i = 0; i < 3; i++)
			{
				...
				vTaskDelay(500);
				xQueueSend(TempQueueHandle, &temperatureValue, 0); // Send ECG signal to the queue value by value
				vTaskDelay(500);
			}
...
/*		for(;;)
		{
			for (int i = 0; i < 3; i++)
			{
				...
				vTaskDelay(500);
				xQueueSend(LightQueueHandle, &lightValue, 0); // Send ECG signal to the queue value by value
				vTaskDelay(500);
			}
	...
	/* Infinite loop */
		for(;;)
		{
			for (int i = 0; i < 10; i++)
			{
				...
				vTaskDelay(500);
				xQueueSend(PollutionQueueHandle, &pollutionValue, 0); // Send ECG signal to the queue value by value
				vTaskDelay(500);
			}
